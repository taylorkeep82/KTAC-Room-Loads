...
def ddy_blocks(ddy_list, tar):
    """
    This function takes a DDY file without blank rows. It picks up the blocks by a special string in the block.
    :param ddy_list: This is the packed ddy list with different blocks
    ...
    ddy_dict = {}          # store data in a dictionary
    for blocks in ddy_list:
        for lines in blocks:
            if tar in lines:
                for l in blocks:
                        y = l.replace(" ", "")
...
def updatesite(packedlist, idf):
    ...
    SiteInformation = ddy_blocks(packedlist, target)
...
def UpdateLocationInfinIDF(idf1, ddyname):
    ...
    """
    sizingprds = idf1.idfobjects['SIZINGPERIOD:DESIGNDAY']
    DDYlist = ['Annual Cooling (DB=>MWB) 1%', 'Annual Cooling (DP=>MDB) 1%', 'Annual Cooling (Enthalpy=>MDB) 1%',
               'Annual Cooling (WB=>MDB) 1%', 'Annual Heating 99%',
               'Annual Heating Wind 99% Design Conditions WS=>MCDB',
               'Annual Humidification 99% Design Conditions DP=>MCDB']
    for prds in range(len(sizingprds)):
        sizingprd = idf1.idfobjects['SIZINGPERIOD:DESIGNDAY'][prds]
        target = DDYlist[prds]
        packedls = remove_blank_lines(ddyname)
        DP = ddy_blocks(packedls, target)
        # print(DP)
...
def ReadHVACdesignLoads(resultpath):
    """
    Reads HVAC design loads from an html results file
    :return: table of loads
    """
    ...
    for table2 in tables:
            if line2 in table2[0]:
                ...
                d['LoadClg' + d['Zonename' + str(i)] + '_V_Latent'] = targettable[10][4]
                d['LoadClg' + d['Zonename' + str(i)] + '_V_Sensible'] = targettable[10][1]
                ...
                d['LoadHtg' + d['Zonename' + str(i)] + '_V_Latent'] = targettable2[10][4]
                d['LoadHtg' + d['Zonename' + str(i)] + '_V_Sensible'] = targettable2[10][1]
                i+=1
    return d


def compare_two_IDF(file1, file2, resultdir):
    blockHVACfile = remove_blank_lines(file1)
    blockidealfile = remove_blank_lines(file2)
    tag = 1
    ...
    for blocks in blockHVACfile:
        for i in range(0, len(blocks)):
            if 'Name' not in blocks[i]:
                b = ddy_blocks(blockidealfile, blocks[i])
                if len(b) != 0:
                    tag = 1
                else:
                    tag = 0
...
if __name__ == '__main__':
    ...
    createFolder('C:\\EnergyPlusV9-0-1\\Results')
    weatherfilename_list, epwDir, ddyDir, LargeIDFLs, SmallIDFLs = setDirectories()
    ...
    for i in weatherfilename_list:
        ...
        GeneratedIDFdir = "C:\\EnergyPlusV9-0-1\\Results\\idfGenerated\\" + i + "\\"
        createFolder(GeneratedIDFdir)
        for j in range(0, len(LargeIDFLs)):
            fname1 = 'C:\\EnergyPlusV9-0-1\\IDFscripting\\Large\\' + LargeIDFLs[j]
            ...
            surfaces = idf1.idfobjects['BuildingSurface:Detailed'.upper()]
            windows = idf1.idfobjects['FenestrationSurface:Detailed'.upper()]
            overhangs = idf1.idfobjects['Shading:Overhang'.upper()]
            fins = idf1.idfobjects['Shading:Fin'.upper()]
            #constructions = ["Katerra", "T24"]

            for surface in surfaces:
                if str(surface.Surface_Type) == "Wall":
                    surface.Construction_Name = i[:-5] + " WALL"
                if str(surface.Surface_Type) == "Roof":
                    surface.Construction_Name = i[:-5] + " ROOF"
            for window in windows:
                if int(i[0]) <= 3:
                    if window.Name[0] == "F":
                        window.Construction_Name = "SOUTH_FIXED_WINDOW"
                    elif window.Name[0] == "S":
                        window.Construction_Name = "SOUTH_SLIDER_WINDOW"
                    else:
                        window.Construction_Name = "SOUTH_DOOR_WINDOW"
                else:
                    if window.Name[0] == "F":
                        window.Construction_Name = "NORTH_FIXED_WINDOW"
                    elif window.Name[0] == "S":
                        window.Construction_Name = "NORTH_SLIDER_WINDOW"
                    else:
                        window.Construction_Name = "NORTH_DOOR_WINDOW"
                # window.Construction_Name = i[:-5] + " WINDOW"

            # WinMaterial.UFactor = '1.81704416'
            # WinMaterial.Name = 'TypicalWindow U0.32 SHGC 0.25'
            ...
            for axis in [180]:
                ...
                for k in range(1):
                    depth_o = 0.001 + k*0.3048/2
                    for overhang in overhangs:
                        overhang.Depth = depth_o
                    for m in range(1):
                        depth_f = 0.001 + m*0.3048
                        for fin in fins:
                            fin.Left_Depth = depth_f
                            fin.Right_Depth = depth_f

                        #for z in range(1):
                         #   ventrate = [0.0, 1.03]
                          #  ventreq = idf1.idfobjects['ZoneVentilation:DesignFlowRate'.upper()]
                           # ventreq.Air_Changes_per_Hour = ventrate[z]
                        LocationIDF = TypeDir + 'Orientation_' + str(axis) + "_" + str(depth_o) + "_" + str(depth_f) + '.idf'
                        idf1.saveas(LocationIDF)

                        # Define the folder path to store the results
                        resultsdir = 'C:\\EnergyPlusV9-0-1\\Results\\' + i + '\\' + LargeIDFLs[j].split('.')[0] + '\\' \
                                     + str(axis) + '\\' + str(depth_o) + '\\' + str(depth_f)
                        ...
                        ClgVSensible.append(d.get('LoadClg' + d.get('Zonename1') + '_V_Sensible'))
                        ClgVLatent.append(d.get('LoadClg' + d.get('Zonename1') + '_V_Latent'))
                        ...
                        HtgVSensible.append(d.get('LoadHtg' + d.get('Zonename1') + '_V_Sensible'))
                        HtgVLatent.append(d.get('LoadHtg' + d.get('Zonename1') + '_V_Latent'))
                        Parameters.append(i + "_" + str(axis) + '_' + str(depth_o) + "_" + str(depth_f) + LargeIDFLs[j].split('.')[0])
                ...
                # resultsdir2 = 'C:\\EnergyPlusV9-0-1\\Results\\' + i + '\\' + LargeIDFLs[j].split('.')[0] + '_Corner' \
                #               + '\\' + str(axis)
            ...
            #     fname_1 = 'C:\\EnergyPlusV9-0-1\\IDFscripting\\Small\\' + SmallIDFLs[k]
            ...
            #         results_dir = 'C:\\EnergyPlusV9-0-1\\Results\\' + i + '\\' + SmallIDFLs[k].split('.')[0] \
            #                       + '\\' + str(Axis)
            ...
            #         results_dir2 = 'C:\\EnergyPlusV9-0-1\\Results\\' + i + '\\' + SmallIDFLs[k].split('.')[0]\
            #                        + '_Corner' + '\\' + str(Axis)
    ...
    CVSS = pd.Series(ClgVSensible)
    CVLS = pd.Series(ClgVLatent)
    HS = pd.Series(HeatingLoads)
    HSS = pd.Series(HtgSensible)
    HLS = pd.Series(HtgLatent)
    HVSS = pd.Series(HtgVSensible)
    HVLS = pd.Series(HtgVLatent)
    ...
    df = pd.DataFrame().assign(CoolingLoads=CS.values, CoolingSensibleLoads=CSS.values, CoolingLatentLoads=CLS.values,
                               CoolingVSLoads=CVSS.values, CoolingVLLoads=CVLS.values,
                               HeatingLoads=HS.values, HeatingSensibleLoads=HSS.values, HeatingLatentLoads=HLS.values,
                               HeatingVSLoads=HVSS.values, HeatingVLLoads=HVLS.values,
                               parameters=ParS.values)
    # print(df)
    df.to_csv(path_or_buf='C:\\EnergyPlusV9-0-1\\Results\\PeakLoads_PARAMETERS.csv')
